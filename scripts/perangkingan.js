...
const API_PERANGKINGAN = `${API_BASE}/api/perangkingan`;
const API_STUDENTS = `${API_PERANGKINGAN}/students`;
...
const loadStudents = async () => {
  const dropdown = $("#pilihSiswa");
  dropdown.empty().append('<option value="">Pilih Siswa</option>');
  ...
  try {
    ...
    if (students.length === 0) {
      dropdown.append('<option value="">Belum ada data siswa</option>');
      return;
    }
    ...
    dropdown.append('<option value="">Gagal memuat data siswa</option>');
  }
};
...
const loadStats = async () => {
  ...
  try {
    const response = await fetchJSON(
      `${API_PERANGKINGAN}/students/${currentStudentId}/stats`
    );
    updateStats(response?.data);
  } catch (error) {
    console.error("[perangkingan] stats", error);
    updateStats(null);
  }
};
...
const loadBestAlternative = async () => {
  ...
  try {
    const response = await fetchJSON(
      `${API_PERANGKINGAN}/students/${currentStudentId}/top`
    );
    ...
    console.error("[perangkingan] top", error);
    updateBestAlternative(null);
  }
};
...
const checkHasData = async () => {
  ...
  try {
    const response = await fetchJSON(
      `${API_PERANGKINGAN}/students/${currentStudentId}/has-data`
    );
    const hasData = Boolean(response?.has_data);
    toggleNoDataState(hasData);
    return hasData;
  } catch (error) {
    console.error("[perangkingan] has-data", error);
    toggleNoDataState(false);
    return false;
  }
};
...
const buildTable = () => {
  dataTable = $("#myTablePerangkingan").DataTable({
    ajax: (_data, callback) => {
      ...
      const request = $.ajax({
        url: `${API_PERANGKINGAN}/students/${currentStudentId}/perangkingan`,
        method: "GET",
        dataType: "json",
        xhrFields: { withCredentials: true },
        success: (response) => {
        ...
        error: (xhr, _status, error) => {
          console.error("[perangkingan] table", error);
          console.error("[perangkingan] response", xhr?.responseText);
          callback({ data: [] });
        },
      });
...
const exportPDF = () => {
  if (!currentStudentId) {
    showToast("warning", "Pilih siswa dulu", "Silakan pilih siswa sebelum ekspor.");
    return;
  }
  ...
  const selectedText = $("#pilihSiswa option:selected").text() || "Siswa";

  const addHeader = () => {
    try {
      doc.addImage(logoPath, "PNG", 15, 10, 20, 20);
    } catch (err) {
      console.warn("[perangkingan] gagal memuat logo untuk PDF", err);
    }
    doc.setFont("helvetica", "bold");
    doc.setFontSize(16);
    doc.text(namaSekolah, doc.internal.pageSize.getWidth() / 2, 18, {
      align: "center",
    });
    doc.setFontSize(12);
  ...
  const filename = selectedText.split(" (")[0] || "perangkingan";
  doc.save(`Laporan_Perangkingan_${filename}.pdf`);
};

const bindEvents = () => {
  $("#pilihSiswa").on("change", async (event) => {
  ...
  $("#refreshData").on("click", async () => {
    if (!currentStudentId) {
      showToast("warning", "Pilih siswa dulu", "Silakan pilih siswa sebelum memuat ulang data.");
      return;
    }
...
$(document).ready(() => {
  initPage().catch((error) => {
    console.error("[perangkingan] init", error);
